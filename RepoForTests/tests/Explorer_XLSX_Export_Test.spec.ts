...
//Login to HiTS Explorer
  ...
  await page.getByText('HiTS Explorer').click();
   ...
   await expect(page).toHaveURL("https://ci.hits.axleresearch.net/hits-explorer")
    ...
    //Export the selected Sample's data as a xlsx
    await page.getByTitle('Export Data').click();
    await page.click('.p-radiobutton_container__text:has-text(".xlsx")');
    ...
    //save the downloaded csv as 'XLSX_Export_Test.xlsx' in the root directory
    await download1.saveAs('XLSX_Export_Test.xlsx');
